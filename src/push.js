...
export function pushBenachrichtigungSenden(titel, text, url, teamName, muteKey) {
  return supabase.functions.invoke("send-push", {
    body: { title: titel, body: text, url: url || "/", teamName: teamName || null, muteKey: muteKey || null },
  }).catch((e) => console.error("Push-Versand fehlgeschlagen:", e));
}
 
export function pushStummschaltungenSynchronisieren(idListe) {
  return supabase.from("push_subscriptions").update({ stummgeschaltete_ids: idListe }).eq("device_id", getDeviceId());
}
